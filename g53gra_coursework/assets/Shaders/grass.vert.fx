...
void main()
{
    ...
    position_worldspace = (modelMatrix * vec4(vertex_position_modelspace + vec3(0.0f, 0.0f, 0.2f * vertex_position_modelspace.y * sin(totalTime)), 1.0)).xyz;

    // Compute the vertex position based on the project/view/model transformation matricies
    gl_Position = projectionMatrix * viewMatrix * modelMatrix * vec4(vertex_position_modelspace + vec3(0.0f, 0.0f, 0.2f * vertex_position_modelspace.y * sin(totalTime)), 1.0);

    // Transform light and eye directions into tangent space
    vec3 normal_cameraspace = normalize(normMatrix * vertex_normal_modelspace);
    vec3 tangent_cameraspace = normalize(normMatrix * vertex_tangent_modelspace);
    vec3 bitangent_cameraspace = normalize(normMatrix * vertex_bitangent_modelspace);
    ...
    normal_tangentSpace = cameraSpaceToTangentSpace * normal_cameraspace;
